...
def extract_frames_from_videos(video_dir = VIDEO_PATH, output_dir  = FRAMES_PATH, processed_dir=PROCESSED_PATH, frames_per_second=10):
...
def extract_frames(video_path, output_folder, frames_per_second=10):
    ...
    while True:
        ...
        if frame_count % interval == 0:
            frame_filename = os.path.join(output_folder, f"{os.path.splitext(os.path.basename(video_path))[0]}_frame_{extracted_frame_count:04d}.jpg")
            cv2.imwrite(frame_filename, frame)
            extracted_frame_count += 1
        frame_count += 1
    video_capture.release()
    print(f"Đa trích xuất {extracted_frame_count} khung hình từ {video_path} vào {output_folder}.")
